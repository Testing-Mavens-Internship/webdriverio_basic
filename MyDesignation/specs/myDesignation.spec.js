...
describe("MyDesignation application: ", () => {
  it("It should lauch the MyDesignation application", async () => {
    ...
    expect(await landingPage.$header().isDisplayed())
      .withContext("Expect the landing page conatin application logo")
      .toBe(true);
  });

  it("Click on the item to be purchase", async () => {
    await landingPage.clickOnItem();
    expect(await selectedProduct.$selectedProductHeader().isDisplayed())
      .withContext(
        "Clicked item should be open with a new page with the selected product Heading"
      )
      .toBe(true);
  });
  it("Select the item size", async () => {
    ...
    expect(await selectedProduct.$productAddConformation().isDisplayed())
      .withContext(
        "Expect the product name should be display on the top left while clicking on the add to cart button"
      )
      .toBe(true);
  });
  it("Click on click to proceed button", async () => {
    await proceedCheckout.clickOnProceedCheckout();
    expect(await proceedCheckout.$billingDetailsHeading().isDisplayed())
      .withContext(
        "Expect the page should display with Billing Details heading"
      )
      .toBe(true);
  });
  it("Click on place order button", async () => {
    ...
    let warning = await billingDetailsPage
      .$$billFormWarning()
      .map((item) => item.getText());
    // let warnings = warning.map((item) => item.replace("is a required field.", ""));
    for (let item of warning) {
      expect(
        await billingDetailsPage.$billFormWarningFields(item).isDisplayed()
      )
        .withContext(
          `Expect the warning contain empty mandatory field "${item}"`
        )
        .toBe(true);
    }
  });
  ...
  it("Click on the first name field and enter the first name", async () => {
    await billingDetailsPage.fillingInputFields(
      firstName,
      lastName,
      address1,
      address2,
    ...
    for (let item of arr) {
      expect(await billingDetailsPage.$inputFields(item).isDisplayed())
        .withContext(
          `Expect show warning of other fields when ${item} only fill and click on the place order`
        )
        .toBe(true);
    }
