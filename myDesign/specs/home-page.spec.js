...
xdescribe("Mydesignation Application automation", () => {
  ...
  it("Click on the desired product and verify navigation", async () => {
    ...
    expect(await productPage.$details("Gojo Co-Ords Set for Men").isDisplayed())
      .withContext("Expect product to be clicked")
      .toBe(true);
  });
  ...
  it("Click on the size of desired product", async () => {
    ...
    expect(await productPage.$addCart("Add to cart").isDisplayed())
      .withContext("Expect product size to be clicked")
      .toBe(true);
  });
  ...
  it("click on add to cart", async () => {
    ...
    expect(
      await productPage
        .$viewText(" Gojo Co-Ords Set for Men” has been added to your cart. ")
        .isDisplayed()
    )
      .withContext("Expect product size to be clicked")
      .toBe(true);
  ...
  it("click on view cart and verify navigation", async () => {
    ...
    expect(await cartPage.$viewCartPage().isDisplayed())
  ...
  it("click on proceed to checkout and verify navigation", async () => {
    ...
    expect(await billingPage.$billing().isDisplayed())
      .withContext("Expect header to be displayed")
      .toBe(true);
  });
  ...
  it("Click place order button", async () => {
    ...
    errorMessage = await billingPage
      .$$verifyFillForm()
      .map((item) => item.getText());
    for (let item of errorMessage) {
      expect(await billingPage.$errorMessages(item).isDisplayed())
  ...
  it("Enter input details", async () => {
    ...
    expect(
      await billingPage
        .$eachErrorMessage("billing_first_name")
  ...
  it("enter pincode and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$eachErrorMessage("Billing PIN Code ")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
  ...
  it("enter phone and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$eachErrorMessage("Billing Phone ")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
  ...
  it("enter email and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$eachErrorMessage("Billing Email address")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
});
