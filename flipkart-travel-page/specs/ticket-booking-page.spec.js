...
describe("Buy a flight ticket for 5 members in flipkart travel page:", () => {
  ...
  it("fill the flight details", async () => {
    ...
    expect(await flightPage.$logo().isDisplayed())
      .withContext("Expect the flipkart logo to be displayed")
      .toBe(true);
    expect(await flightPage.$verifyDetails(data.from).waitForDisplayed())
      .withContext("Expect the depart place to be displayed")
      .toBe(true);
    expect(await flightPage.$verifyDetails(data.to).waitForDisplayed())
  ...
  it("verify the flights", async () => {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      expect(await flightPage.$flightDetail(data.fromCode, i).isDisplayed())
        .withContext("Expect BOM to be displayed")
        .toBe(true);
      expect(await flightPage.$flightDetail(data.toCode, i).isDisplayed())
        .withContext("Expect BOM to be displayed")
        .toBe(true);
    }
  });
  ...
  it("Apply filter and sort the price details", async () => {
    ...
    expect(await flightPage.sortPrice())
      ...
      .toBe(true);
    expect(await flightPage.verifyTime(data.time[1]))
