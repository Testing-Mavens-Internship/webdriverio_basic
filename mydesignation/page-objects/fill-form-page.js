...
class FillFormPage extends CommonPage{
    constructor(){
        ...
        this.$verifyErrorMessages = (name) => $(`//li[text()= "The Phone Number should contain only 10 digits."or(text()=" is a required field." and strong[text()="${name}"] )]`)
        this.$verifyEachErrorMessage = (error) => $(`//strong[text()="${error}"]`)
        this.$verifyStateErrorMessage = (state) => $(`//span//select[@id="billing_state"]/option[text()="${state}"]`)
    }
    /**
