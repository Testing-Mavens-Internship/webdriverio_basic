...
describe("End to end flow for the application mydesignation", () => {
  ...
  it("enter last name and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$verifyEachErrorMessage("Billing Last name")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
  ...
  it("select state and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$verifyEachErrorMessage("Billing State")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
  ...
  it("enter pincode and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$verifyEachErrorMessage("Billing PIN Code ")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
  ...
  it("enter phone and verify error message", async () => {
    ...
    expect(
      await fillForm
        .$verifyEachErrorMessage("Billing Phone ")
        .waitForDisplayed({ timeout: 5000, reverse: true })
    );
  });
