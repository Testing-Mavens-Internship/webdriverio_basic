...
class ItemPage extends CommonPage{
    constructor(){
        super();
        this.$sizeOfItem = (type,value) => $(`//label[text()='${type}']/../following-sibling::span//span[text()='${value}']/..`);
        this.$messageBox = () => $(`//div[@class="woocommerce-notices-wrapper"]//div[contains(text(),' added to your cart')]`);
    }
    /**
     * select item sizes
    ...
    async clickOnItemSize(tshirtSize, shortsSize){
        await this.$sizeOfItem('T-shirt Size', tshirtSize).scrollIntoView({ block : 'center'});
