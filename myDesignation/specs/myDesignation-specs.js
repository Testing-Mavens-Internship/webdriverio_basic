...
describe("MYDESIGNATION automation", () => {
  ...
  it("click on a product and validate navigation ", async () => {
    ...
    expect(
      await productPage
        .$productPageHeading("Gojo Co-Ords Set for Men")
        .isDisplayed()
    )
  ...
  it("Select the size of of the product and click on Add to cart Button.Validate navigation", async () => {
    ...
    expect(await cartPage.$productNameInCart(product).isDisplayed())
      .withContext("product name does not match")
      .toBe(true);
  });
  ...
  it("Click on place order button without filling mandatory fields", async () => {
    ...
    errorArray = await billingDetailsPage
      .$$errorMessages()
      .map((item) => item.getText());
    for (let item of errorArray) {
      expect(await billingDetailsPage.$errorMessage(item).isDisplayed())
        .withContext("text is not displayed")
        .toBe(true);
    }
  });
  it("Click on place order without filling one mandatory field each", async () => {
    ...
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_first_name",
      fname
    );
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing First name")
    ...
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing Last name")
        .waitForDisplayed({ timeout: 2000, reverse: true })
    );
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_address_1",
    ...
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing Street address")
        .waitForDisplayed({ timeout: 2000, reverse: true })
    );
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_address_2",
    ...
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing Town / City")
    ...
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing PIN Code")
        .waitForDisplayed({ timeout: 20000, reverse: true })
    );
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_phone",
    ...
    expect(
      await billingDetailsPage
        .$errorMessagePhone("Billing Phone")
        .waitForDisplayed({ timeout: 20000, reverse: true })
    );
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_email",
    ...
    expect(
      await billingDetailsPage
        .$errorMessageNew("Billing Email address")
        .waitForDisplayed({ timeout: 20000, reverse: true })
    );
    await billingDetailsPage.clickOnPlaceorderWithoutFIllingOneMandatoryFIeldEach(
      "billing_email",
