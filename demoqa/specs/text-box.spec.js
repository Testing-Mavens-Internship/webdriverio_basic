...
xdescribe("Demo QA Application Text Box automation", () => {
  ...
  it("CLick on the full name and enter the values", async () => {
    ...
    expect(await textBoxPage.$elementName("userName").getValue()).toBe(
      "Sreerag"
    );
  ...
  it("Verify the  name", async () => {
    expect(await textBoxPage.$validatingfields("Sreerag").isDisplayed()).toBe(
      true
    );
  });
  ...
  it("Verify the email", async () => {
    expect(
      await textBoxPage.$validatingfields("tester@gmail.com").isDisplayed()
    ).toBe(true);
  });
