...
xdescribe("Demo QA Application Text Box automation", () => {
  ...
  it("CLick on submit button and validate the entered data",async () => {
    ...
    expect(await textBoxPage.$validate("name").getText()).withContext("Expect permanent address to be displayed").toContain(fullName)
    expect(await textBoxPage.$validate("email").getText()).withContext("Expect permanent address to be displayed").toContain(email)
    expect(await textBoxPage.$validate("currentAddress").getText()).withContext("Expect permanent address to be displayed").toContain(address1)
