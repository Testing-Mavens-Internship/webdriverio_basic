...
xdescribe("Demo QA Application Web Table automation", () => {
    ...
    it("Fill registration form",async () => {
      ...
      await webTablesPage.registerForm(firstName,lastName,email,age,salary,department)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect first name to be displayed").toContain(firstName)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect last name to be displayed").toContain(lastName)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect email to be displayed").toContain(email)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect age to be displayed").toContain(age)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect salary to be displayed").toContain(salary)
        expect(await webTablesPage.$addToTable().getText()).withContext("Expect department to be displayed").toContain(department)
