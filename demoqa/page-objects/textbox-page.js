class TextPage {
    constructor() {
        ...
        this.$validate = (name) => $(`//div[@class="border col-md-12 col-sm-12"]//p[@id="${name}"]`)
    }
    /**
     * Enter full name
     ...
     * @param {string} inputValue 
     */
    async enterEmail(inputValue,email) {
        await this.$input(inputValue).setValue(email)
     ...
     */
    async enterCurrentAddress(address,address1) {
        await this.$current(address).setValue(address1)
        await this.$current("Permanent Address").waitForDisplayed({timeout: 20000});
    }
    /**
     * enter permenant address
     * @param {string} address 
     */
    async enterPermenantAddress(address,address2) {
        await this.$current(address).setValue(address2)
