...
class HomePage extends Common {
  constructor() {
    ...
    this.$sortDropDown = () =>
      $(
        '//select[@class="product_sort_container"]/option[text()="Price (high to low)"]'
      );
    this.$$price = () => $$('//div[@class="inventory_item_price"]');
    this.$addToCart = (productName) =>
      $(
        `//div[text()="${productName}"]//ancestor::div//div//button[text()="Add to cart"]`
      );
    this.$verifyRemove = (a) =>
      $(`//div[text()="${a}"]/ancestor::div//button[text()="Remove"]`);
