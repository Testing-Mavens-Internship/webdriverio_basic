...
class TravelPage extends CommonPage {
  ...
  async fillUpDetails(from, to, month, day, traveller, cabin, elementTodisplay) {
    ...
    await this.$selectField(to).waitForClickable({ timeout: 2000 });
    ...
    await this.$loadText('Looking for flights...').waitForDisplayed({ timeout : 3000 });
    await elementTodisplay.waitForDisplayed({ timeout : 3000 });
