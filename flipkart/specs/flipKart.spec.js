...
describe("End to end autoamtion for flight booking in flipkart", ()=>{
    ...
    it("Select the departure daate",async()=>{
        ...
        expect(await travelPage.$fromToDepartReturnClass("03")).toHaveTextContaining("Depart On");
    });
    ...
    it("Click on the search flight", async()=>{
        await travelPage.clickOnSearch();
        expect (await fligthDetails.$filterByText().waitForDisplayed({timeout : 5000})).withContext("Loaded the flight details").toBe(true)
    });
    ...
    it("Click on the non-stop flight",async()=>{
        ...
        .withContext("The spice jet travels is selected for the traveling is selected")
        .toBe(true);
    });
