...
describe('Swag Labs application',()=>{
    ...
    it("It should lauch the Swag Labs application",async ()=>{
        ...
        expect(await homePage.$header().isDisplayed()).withContext("Expect Swag Labs title should be display").toBe(true)
    })
    it("Fill the credentials",async()=>{
        await homePage.login(userName,password)
        expect(await homePage.$header().isDisplayed()).withContext("Expect the Swag Labs title on the home page").toBe(true)
       expect(await homePage.$productTitle(title1).isDisplayed()).withContext("Expect the Product title on the homepage").toBe(true) 
    })
