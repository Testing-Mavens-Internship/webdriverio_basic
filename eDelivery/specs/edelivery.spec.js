...
expect(await landingPage.$header().isDisplayed()).withContext("Header is not displayed").toBe(true)

})
it("Click on login button and Register button",async()=>{
  await landingPage.register();
  expect (await landingPage.$logoValidation().isDisplayed()).withContext("Login pop up is not displayed").toBe(true)
  expect (await landingPage.$registerValidation().isDisplayed()).withContext("Register pop up is not displayed").toBe(true)
})
it("Enter details to register",async()=>{
  await registerForm.fillForm(firstName,lastName,firstName,phoneNumber);
  expect(await registerForm.$loginMessage().isDisplayed()).withContext("Unsuccessful login").toBe(true)
  expect(await registerForm.$nameValidation(firstName).isDisplayed()).withContext("First name is not displayed").toBe(true)
